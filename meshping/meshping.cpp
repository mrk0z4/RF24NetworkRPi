...
void loop(void)
{
  ...
  if ( now - last_time_sent >= interval )
  {
    ...
    if (ok)
    {
      printf("%lu: APP Send ok\n\r",millis());
    }
    else
    {
      printf("%lu: APP Send failed\n\r",millis());
...
bool send_T(uint16_t to)
{
  ...
  printf("---------------------------------\n\r");
  printf("%lu: APP Sending %lu to 0%o...\n\r",millis(),message,to);
...
bool send_N(uint16_t to)
{
  ...
  printf("---------------------------------\n\r");
  printf("%lu: APP Sending active nodes to 0%o...\n\r",millis(),to);
...
void handle_T(RF24NetworkHeader& header)
{
  ...
  printf("%lu: APP Received %lu from 0%o\n\r",millis(),message,header.from_node);
...
void handle_N(RF24NetworkHeader& header)
{
  ...
  while ( i < max_active_nodes && incoming_nodes[i] > 00 ){
    ...
    printf("Node added > %i",incoming_nodes[i++]);
  }
}
...
unsigned long millis(){
	return (unsigned long) ((c * 1000) / CLOCKS_PER_SEC);
}
